...
def map_plot():
    ...
    while not window_closed():
        ...
        if start != None:
            start.draw_vertex(1, 0, 0) #if the start vertex is found, draw the start vertex (so user can see)
        ...
        if path_list != None: 
            ...
            for vertex in path_list:
                if vertex.back != None: #if-statement to prevent unnecessary 
                    ...
                    vertex.draw_edge(vertex.back, 1, 0, 0) #draws edge from vertex to its back-pointer
                    vertex.draw_vertex(1, 0, 0) #draws the vertex of points in path list
                
                start.draw_vertex(1, 0, 0) #draws the start vertex again since our for-loop inevitably does not include it
